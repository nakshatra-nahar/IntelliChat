...
<a href="https://youtu.be/0jzILPqA454?si=6d13VCmfCt3iQa9W">
...
The outgoing intelli Chat request body is encapsulated in one of the following formats:
...
<a href="https://youtu.be/0jzILPqA454?si=0_FRD3mpmqnNFf5n&t=16">
  <img src={YoutubeLogo} className={'youtube-icon'} />
  Video demo
</a>
...
the server. See [`example`](https://intellichat.dev/examples/servers) server code. <br />
...
```html
<intelli-chat stream="true"></intelli-chat>
```
...
```html
<intelli-chat stream='{"simulation": 6}'></intelli-chat>
```
...
Object used to configure the outgoing request settings. It MUST have either `url` or `handler` defined. <br />
...
It is important to note that exchanged messages must be Stringified JSONs where intelli Chat will send its messages using the [Request message](connect) format
and the server must send its messages using the [`Response`](#Response) format. Example messages: <br />
intelli Chat message: <br />
`'{"messages":[{"role":"user","text":"Message from intelli Chat"}]}'` <br />
Server message: <br />
...
```html
<intelli-chat request='{"url": "ws://customapi.com", "websocket": true}'></intelli-chat>
```
...
:::tip
Check the [websocket server template](https://github.com/OvidijusParsiunas/intelli-chat/tree/main/example-servers/node/websockets) to help you get started.
:::
...
`signals` is a map of functions which are used to notify intelli Chat on the status of the request and its result. The available
...
chatElementRef.request = {
  ...
  handler: (_, signals) => {
    try {
      ...
      websocket.onerror = () => {
        // 'Connection error' is a special string that will also display in intelli Chat
...
Object containing functions that are used to notify the intelli Chat component about the status of the current request.
The `stopClicked` and `newUserMessage` functions are triggered by intelli Chat itself and contain `listener` properties
