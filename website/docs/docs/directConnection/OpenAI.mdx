...
When set to `true` or the `assistant_id` is not defined, intelli Chat will automatically create a new assistant when the user sends the first message. <br />
...
- Type: \{<br />
  ...
  &nbsp;&nbsp;&nbsp;&nbsp; `fileIds?: string[]`, <br />
  &nbsp;&nbsp;&nbsp;&nbsp; `tools?`: \{ <br />
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; `type?`: `"code_interpreter"` | `"retrieval"` | `"function"`, <br />
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; `function?`: \{`name: string`, `description?: string`, `parameters?: object`\} <br />
  \}\}

- Default: _\{model: "gpt-4"\}_

When `assistant_id` is not used, this object is used to define the details of the new assistant that will be created by intelli Chat when
...
`fileIds` is an array of file ids that the new assistant has access to. Log into OpenAI and check [files](https://platform.openai.com/files). <br />
...
:::info
Access the newly created `assistant_id` via `chatElementRef._activeService.rawBody.assistant_id`.
:::
...
Connect to Open AI's [`Images`](https://platform.openai.com/docs/api-reference/images) API. You can set this property to _true_ or configure it using an object: <br />
`model` is the name of the specific model to be used by the API. Check [/v1/images/generations](https://platform.openai.com/docs/models/model-endpoint-compatibility) for more. <br />
...
Try it out live in the intelli Chat [Playground](/playground).
