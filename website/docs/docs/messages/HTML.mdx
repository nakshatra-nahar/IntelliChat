...
<BrowserOnly>
  {() => {
    require('active-table-react').intelliChat;
  }}
</BrowserOnly>
<BrowserOnly>
  {() => {
...
```html
<intelli-chat initialMessages='[{"html": "<button>Button</button>", "role": "user"}]'></intelli-chat>
```
...
Because intelli Chat is a [shadow element](https://developer.mozilla.org/en-US/docs/Web/API/Web_components/Using_shadow_DOM) and your html is rendered inside it - the resulting elements will not be able to access the CSS and JavaScript
...
### intelli Chat Classes {#intelliChatClasses}

intelli Chat comes with a pre-defined set of classes that can be used to add styling/functionality to your elements. <br />
`intelli-chat-button` - applies default button styling. <br />
`intelli-chat-suggestion-button` - when clicked sends the text that is inside the button as a new user message. <br />
`intelli-chat-temporary-message` - removes the message when a new one is added to the chat. This class needs to be applied
...
<ComponentContainer>
  <intelliChatBrowser
    initialMessages={[
      {html: '<button class="intelli-chat-button intelli-chat-suggestion-button">Hello</button>', role: 'ai'},
      {
        html: '<button class="intelli-chat-suggestion-button intelli-chat-temporary-message">Goodbye</button>',
        role: 'user',
      },
    ]}
    style={{height: '370px', borderRadius: '8px'}}
...
chatElementRef.initialMessages = [
  {html: '<button class="intelli-chat-button intelli-chat-suggestion-button">Hello</button>', role: 'ai'},
  {html: '<button class="intelli-chat-suggestion-button intelli-chat-temporary-message">Goodbye</button>', role: 'user'},
];
```
...
:::info
If you have suggestions for new classes that you think would be useful, please raise an [issue](https://github.com/OvidijusParsiunas/intelli-chat/issues) on GitHub.
:::
...
To unset/change the message bubble styling, use the `html` property in the [messageStyles](/docs/styles#messageStyles) object.
...
```html
<intelli-chat messageStyles='{"html": {"shared": {"bubble": {"backgroundColor": "unset"}}}}'></intelli-chat>
```
...
- Web components can ultimately be created in any way you desire using any framework outside of your project and imported the same ways as you do
  `intelli-chat`. To note, this approach may require you to first import or use them in your app's code in order for your bundler to register them.
...
// HTML
<intelli-chat initialMessages='[{"html": "<custom-element/>", "role": "user"}]'></intelli-chat>
```
...
:::info
If you are experiencing problems with embedding your custom elements inside the chat, you can always raise an [issue](https://github.com/OvidijusParsiunas/intelli-chat/issues) on GitHub.
...
Basic suggestion buttons using [intelli Chat Classes](#intelliChatClasses).
...
<ComponentContainer>
  <intelliChatBrowser
    ...
    initialMessages={[
      ...
      {
        html: `
          <div class="intelli-chat-temporary-message">
            <button class="intelli-chat-button intelli-chat-suggestion-button" style="margin-top: 5px">What do shrimps eat?</button>
            <button class="intelli-chat-button intelli-chat-suggestion-button" style="margin-top: 6px">Can a shrimp fry rice?</button>
            <button class="intelli-chat-button intelli-chat-suggestion-button" style="margin-top: 6px">What is a pistol shrimp?</button>
          </div>`,
        role: 'ai',
      },
    ]}
  ...
  <intelliChatBrowser
    ...
    initialMessages={[
      ...
      {text: 'Have you drank water?', role: 'ai'},
      {
        html: `
          <div class="intelli-chat-temporary-message">
            <button class="intelli-chat-button intelli-chat-suggestion-button" style="border: 1px solid green">Yes</button>
            <button class="intelli-chat-button intelli-chat-suggestion-button" style="border: 1px solid #d80000">No</button>
          </div>`,
        role: 'user',
      },
    ]}
...
Add a chart component (e.g. using [Google Chart](https://developers.google.com/chart)). Live example for [React](https://codesandbox.io/s/intelli-chat-react-chart-ztsgrd?file=/src/App.tsx).
