    ...
    setTimeout(() => {
      if (containerRef.current) {
        ...
        if (containerRef.current && eventTextRef.current) {
          const intelliChatReference = extractChildChatElement(containerRef.current.children[0]);
          intelliChatReference[propertyName] = eventTextRef.current?.updateText;
        } else {
          const intelliChatReference = extractChildChatElement(syncReference.children[0]);
          intelliChatReference[propertyName] = () => {};
        }
      }
