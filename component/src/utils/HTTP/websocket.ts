...
export class Websocket {
  ...
  public static createConnection(io: ServiceIO, messages: Messages) {
    if (!document.body.contains(io.intelliChat)) return; // check if element is still present
    ...
    try {
      ...
      io.websocket.onopen = () => {
        ...
        io.intelliChat._validationHandler?.();
      };
  ...
  private static assignListeners(io: ServiceIO, ws: WebSocket, messages: Messages) {
    ...
    ws.onmessage = async (message) => {
      ...
      try {
        ...
        const finalResult = (await io.intelliChat.responseInterceptor?.(result)) || result;
        const resultData = await io.extractResultData(finalResult);
        if (!resultData || typeof resultData !== 'object')
          throw Error(ErrorMessages.INVALID_RESPONSE(result, 'server', !!io.intelliChat.responseInterceptor, finalResult));
  ...
  public static async sendWebsocket(io: ServiceIO, body: object, messages: Messages, stringifyBody = true) {
    ...
    const {body: interceptedBody, error} = await RequestUtils.processRequestInterceptor(io.intelliChat, requestDetails);
