...
export class MessagesBase {
  ...
  constructor(intelliChat: intelliChat) {
    ...
    this.messageStyles = intelliChat.messageStyles;
    this._remarkable = RemarkableConfig.createNew();
    this._avatars = intelliChat.avatars;
    this._names = intelliChat.names;
    this._onMessage = FireEvents.onMessage.bind(this, intelliChat);
    if (intelliChat.htmlClassUtilities) this.htmlClassUtilities = intelliChat.htmlClassUtilities;
    setTimeout(() => {
      this.submitUserMessage = intelliChat.submitUserMessage; // wait for it to be available
    });
  }
  ...
  protected static isTemporaryElement(elements: MessageElements) {
    return (
      ...
      HTMLintelliChatElements.isElementTemporary(elements)
    );
  }
