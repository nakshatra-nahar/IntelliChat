...
import {HTMLintelliChatElements} from './htmlintelliChatElements';
...
export class HTMLUtils {
  ...
  private static applyClassUtilitiesToElement(element: HTMLElement, classUtility: HTMLClassUtility) {
    ...
    // if intelli chat class then style was already applied
    if (styles && !HTMLintelliChatElements.doesElementContainintelliChatClass(element)) {
  ...
  public static apply(messages: MessagesBase, outmostElement: HTMLElement) {
    HTMLintelliChatElements.applyintelliChatUtilities(messages, messages.htmlClassUtilities, outmostElement);
