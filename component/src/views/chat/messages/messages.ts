...
import {HTMLintelliChatElements} from './html/htmlintelliChatElements';
...
export class Messages extends MessagesBase {
  ...
  constructor(intelliChat: intelliChat, serviceIO: ServiceIO, panel?: HTMLElement) {
    super(intelliChat);
    const {permittedErrorPrefixes, introPanelMarkUp, demo} = serviceIO;
    this._errorMessageOverrides = intelliChat.errorMessages?.overrides;
    this._onClearMessages = FireEvents.onClearMessages.bind(this, intelliChat);
    this._onError = FireEvents.onError.bind(this, intelliChat);
    this._displayLoadingMessage = Messages.getDisplayLoadingMessage(intelliChat, serviceIO);
    this._permittedErrorPrefixes = permittedErrorPrefixes;
    if (!this.addSetupMessageIfNeeded(intelliChat, serviceIO)) {
      this.populateIntroPanel(panel, introPanelMarkUp, intelliChat.introPanelStyle);
    }
    this.addIntroductoryMessage(intelliChat, serviceIO);
    this.populateHistory(intelliChat);
    this._displayServiceErrorMessages = intelliChat.errorMessages?.displayServiceErrorMessages;
    intelliChat.getMessages = () => JSON.parse(JSON.stringify(this.messages));
    intelliChat.clearMessages = this.clearMessages.bind(this, serviceIO);
    intelliChat.refreshMessages = this.refreshTextMessages.bind(this);
    intelliChat.scrollToBottom = ElementUtils.scrollToBottom.bind(this, this.elementRef);
    intelliChat.addMessage = (message: ResponseI, isUpdate?: boolean) => {
    ...
    if (intelliChat.textToSpeech) {
      TextToSpeech.processConfig(intelliChat.textToSpeech, (processedConfig) => {
  ...
  private static getDisplayLoadingMessage(intelliChat: intelliChat, serviceIO: ServiceIO) {
    if (serviceIO.websocket) return false;
    return intelliChat.displayLoadingBubble ?? true;
  }
  ...
  private addSetupMessageIfNeeded(intelliChat: intelliChat, serviceIO: ServiceIO) {
    const text = SetupMessages.getText(intelliChat, serviceIO);
    if (text) {
  ...
  // WORK - const file for intelli chat classes
  private addIntroductoryMessage(intelliChat?: intelliChat, serviceIO?: ServiceIO) {
    if (intelliChat?.shadowRoot) this._introMessage = intelliChat.introMessage;
    ...
    if (introMessage) {
      ...
      if (elements) {
        ...
        elements.outerContainer.classList.add('intelli-chat-intro');
      }
    }
  }
  ...
  public removeIntroductoryMessage() {
    ...
    if (introMessage.outerContainer.classList.contains('intelli-chat-intro')) {
      introMessage.outerContainer.remove();
      this.messageElementRefs.shift();
    }
  }

  private populateHistory(intelliChat: intelliChat) {
    const history = intelliChat.history || Legacy.processHistory(intelliChat);
    if (!history) return;
    ...
    // https://github.com/OvidijusParsiunas/intelli-chat/issues/84
  ...
  public addNewMessage(data: ResponseI, isHistory = false) {
    ...
    if (message.html !== undefined && message.html !== null) {
      ...
      if (HTMLintelliChatElements.isElementTemporary(elements)) delete message.html;
    }
  ...
  // WORK - update all message classes to use intelli-chat prefix
