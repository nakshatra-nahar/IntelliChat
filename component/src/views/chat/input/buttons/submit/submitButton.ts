...
export class SubmitButton extends InputButton<Styles> {
  ...
  // prettier-ignore
  constructor(intelliChat: intelliChat, inputElementRef: HTMLElement, messages: Messages, serviceIO: ServiceIO,
      fileAttachments: FileAttachments, buttons: Buttons) {
    const submitButtonStyles = SubmitButtonStateStyle.process(intelliChat.submitButtonStyles);
    ...
    intelliChat.disableSubmitButton = this.disableSubmitButton.bind(this, serviceIO);
    this.attemptOverwriteLoadingStyle(intelliChat);
    if (buttons.microphone) this.setUpSpeechToText(buttons.microphone.button, intelliChat.speechToText);
    setTimeout(() => { // in a timeout as intelliChat._validationHandler initialised later
      this._validationHandler = intelliChat._validationHandler;
  ...
  // prettier-ignore
  private attemptOverwriteLoadingStyle(intelliChat: intelliChat) {
    if (this._customStyles?.submit?.svg
        || this._customStyles?.loading?.svg?.content || this._customStyles?.loading?.text?.content) return;
    if (intelliChat.displayLoadingBubble === undefined || intelliChat.displayLoadingBubble === true) {
      ...
      intelliChat.shadowRoot?.appendChild(styleElement);
  ...
  private setUpSpeechToText(microphoneButton: InputButton, speechToText: intelliChat['speechToText']) {
