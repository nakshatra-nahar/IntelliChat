...
export class SpeechToText extends MicrophoneButton {
  ...
  constructor(intelliChat: intelliChat, textInput: TextInputEl, addErrorMessage: AddErrorMessage) {
    super(typeof intelliChat.speechToText === 'object' ? intelliChat.speechToText?.button : {});
    const {serviceName, processedConfig} = this.processConfiguration(textInput, intelliChat.speechToText);
    ...
    if (serviceName === 'webspeech' && !SpeechToElement.isWebSpeechSupported()) {
      ...
      const isInputEnabled = !intelliChat.textInput || !intelliChat.textInput.disabled;
