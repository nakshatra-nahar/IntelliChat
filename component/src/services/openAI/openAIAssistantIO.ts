...
export class OpenAIAssistantIO extends DirectServiceIO {
  ...
  constructor(intelliChat: intelliChat) {
    ...
    super(intelliChat, OpenAIUtils.buildKeyVerificationDetails(), OpenAIUtils.buildHeaders, apiKey);
    ...
    if (typeof config === 'object') {
      ...
      const {function_handler} = intelliChat.directConnection?.openAI?.assistant as OpenAIAssistant;
  ...
  private async fetchHistoryFunc() {
    setTimeout(() => this.intelliChat.disableSubmitButton(), 2); // not initialised when fetchHistoryFunc called
    try {
      const threadMessages = await this.getThreadMessages(this.sessionId as string, true);
      this.intelliChat.disableSubmitButton(false);
  ...
  private async getThreadMessages(thread_id: string, isHistory = false) {
    ...
    if (!isHistory && this.intelliChat.responseInterceptor) {
      threadMessages = (await this.intelliChat.responseInterceptor?.(threadMessages)) as OpenAIAssistantMessagesResult;
    }
  ...
  async extractPollResultData(result: OpenAIRunResult): PollResult {
    ...
    if (status === 'completed' && this.messages) {
      ...
      setTimeout(() => {
        threadMessages.forEach((message) => this.intelliChat.addMessage(message));
      });
  ...
  private async handleTools(toolCalls: ToolCalls): PollResult {
    if (!this._functionHandler) {
      throw Error(
        'Please define the `function_handler` property inside' +
          ' the [openAI](https://intellichat.dev/docs/directConnection/openAI#Assistant) object.'
      );
    }
    ...
    if (!Array.isArray(handlerResponse) || handlerResponse.find((response) => typeof response !== 'string')
      ...
      throw Error('Response must contain an array of strings for each individual function/tool_call, '
        + 'see https://intellichat.dev/docs/directConnection/OpenAI/#assistant-functions.');
    }
