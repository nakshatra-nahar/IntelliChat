...
export class OpenAIChatIO extends DirectServiceIO {
  ...
  constructor(intelliChat: intelliChat) {
    ...
    if (typeof config === 'object') {
      ...
      const {function_handler} = intelliChat.directConnection?.openAI?.chat as OpenAIChat;
  ...
  private async handleTools(tools: ToolAPI, fetchFunc?: FetchFunc, prevBody?: OpenAIChat): Promise<ResponseI> {
    ...
    if (!tools.tool_calls || !fetchFunc || !prevBody || !this._functionHandler) {
      throw Error(
        'Please define the `function_handler` property inside' +
          ' the [openAI](https://intellichat.dev/docs/directConnection/openAI#Chat) object.'
      );
    }
    ...
    if (handlerResponse.text) {
      ...
      return await this.intelliChat.responseInterceptor?.(response) || response;
    }
    ...
    if ((Array.isArray(handlerResponse) && !handlerResponse.find((response) => typeof response !== 'string'))
      ...
      try {
        ...
        result = await this.intelliChat.responseInterceptor?.(result) || result;
        if (result.error) throw result.error.message;
        return {text: result.choices[0].message.content || ''};
      } catch (e) {
    ...
    throw Error(
      'Response object must either be {response: string}[] for each individual function ' +
        'or {text: string} for a direct response, see https://intellichat.dev/docs/directConnection/OpenAI#FunctionHandler.'
    );
  }
}
