...
export class CohereSummarizationIO extends CohereIO {
  constructor(intelliChat: intelliChat) {
    const config = intelliChat.directConnection?.cohere?.summarization as Cohere['summarization'];
    const apiKey = intelliChat.directConnection?.cohere;
    super(intelliChat, 'https://api.cohere.ai/v1/summarize', 'Insert text to summarize', config, apiKey);
  }
