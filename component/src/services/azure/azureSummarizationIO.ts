...
export class AzureSummarizationIO extends AzureLanguageIO {
  ...
  constructor(intelliChat: intelliChat) {
    const config = intelliChat.directConnection?.azure?.summarization as NonNullable<Azure['summarization']>;
    const apiKey = intelliChat.directConnection?.azure;
    super(intelliChat, AzureUtils.buildSummarizationHeader, config.endpoint, apiKey);
