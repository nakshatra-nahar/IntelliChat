...
export class AzureSpeechToTextIO extends AzureSpeechIO {
  ...
  constructor(intelliChat: intelliChat) {
    const config = intelliChat.directConnection?.azure?.speechToText as NonNullable<Azure['speechToText']>;
    const apiKey = intelliChat.directConnection?.azure;
    const defaultFile = {audio: {files: {acceptedFormats: '.wav,.ogg'}}};
    super(intelliChat, AzureUtils.buildSpeechToTextHeaders, config.region, apiKey, defaultFile);
