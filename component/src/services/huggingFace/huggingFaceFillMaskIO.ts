...
export class HuggingFaceFillMaskIO extends HuggingFaceIO {
  ...
  constructor(intelliChat: intelliChat) {
    const config = intelliChat.directConnection?.huggingFace?.fillMask as NonNullable<HuggingFace['fillMask']>;
    const apiKey = intelliChat.directConnection?.huggingFace;
    super(intelliChat, 'The goal of life is [MASK].', 'bert-base-uncased', config, apiKey);
  }
