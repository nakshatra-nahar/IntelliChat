...
export class HuggingFaceQuestionAnswerIO extends HuggingFaceIO {
  ...
  constructor(intelliChat: intelliChat) {
    const config = intelliChat.directConnection?.huggingFace?.questionAnswer as NonNullable<HuggingFace['questionAnswer']>;
    const apiKey = intelliChat.directConnection?.huggingFace;
    super(intelliChat, 'Ask a question', 'bert-large-uncased-whole-word-masking-finetuned-squad', config, apiKey);
