...
export class StabilityAIImageToImageMaskingIO extends StabilityAIIO {
  ...
  constructor(intelliChat: intelliChat) {
    const directConnectionCp = JSON.parse(JSON.stringify(intelliChat.directConnection));
    const apiKey = directConnectionCp?.stabilityAI;
    const defaultFile = {images: {files: {acceptedFormats: '.png', maxNumberOfFiles: 2}}};
    super(intelliChat, StabilityAIUtils.buildKeyVerificationDetails(), StabilityAIUtils.buildHeaders, apiKey, defaultFile);
