...
export class BaseServiceIO implements ServiceIO {
  ...
  constructor(intelliChat: intelliChat, existingFileTypes?: ServiceFileTypes, demo?: DemoT) {
    this.intelliChat = intelliChat;
    this.demo = demo;
    Object.assign(this.rawBody, intelliChat.connect?.additionalBodyProps);
    this.totalMessagesMaxCharLength = intelliChat?.requestBodyLimits?.totalMessagesMaxCharLength;
    this.maxMessages = intelliChat?.requestBodyLimits?.maxMessages;
    SetFileTypes.set(intelliChat, this, existingFileTypes);
    if (intelliChat.connect) this.connectSettings = intelliChat.connect;
    if (this.demo) this.connectSettings.url ??= Demo.URL;
    if (this.connectSettings.websocket) Websocket.setup(this);
    this.stream = this.intelliChat.connect?.stream || Legacy.checkForStream(this.intelliChat);
  }
