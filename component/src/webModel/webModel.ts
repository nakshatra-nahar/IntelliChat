...
// import * as WebLLM2 from 'intelli-chat-web-llm';
...
export class WebModel extends BaseServiceIO {
  ...
  private static readonly GENERIC_ERROR =
    ...
    '[troubleshooting](https://intellichat.dev/docs/webModel#troubleshooting) section of documentation for help.';
  ...
  constructor(intelliChat: intelliChat) {
    ...
    if (typeof intelliChat.webModel === 'object') this._webModel = intelliChat.webModel;
    if (this._webModel.load?.clearCache) WebModel.clearAllCache();
    this.findModelInWindow(intelliChat);
    this.canSendMessage = this.canSubmit.bind(this);
    this._chatEl = intelliChat.shadowRoot?.children[0] as HTMLElement;
    if (intelliChat.history) WebModel.setUpHistory(this._conversationHistory, intelliChat.history);
  }
  ...
  private findModelInWindow(intelliChat: intelliChat, seconds = 0) {
    if (window.webLLM) {
      this.configureInit(this.shouldAddIntroMessage(intelliChat.introMessage));
    } else if (seconds > WebModel.MODULE_SEARCH_LIMIT_S) {
      this._messages?.addNewErrorMessage('service', WebModel.WEB_LLM_NOT_FOUND_ERROR);
      console.error(
        'The intelli-chat-web-llm module has not been attached to the window object. ' + 'Please see the following guide:'
      );
      console.error('https://intellichat.dev/examples/externalModules');
    } else {
      setTimeout(() => this.findModelInWindow(intelliChat, seconds + 1), 1000);
    }
  }
  ...
  private async loadModel(chat: WebLLM.ChatInterface, files?: FileList) {
    ...
    this.intelliChat._validationHandler?.();
  ...
  private async immediateResp(messages: Messages, text: string, chat: WebLLM.ChatInterface) {
    ...
    const response = await WebModel.processResponse(this.intelliChat, messages, output);
  ...
  private async streamResp(messages: Messages, text: string, chat: WebLLM.ChatInterface) {
    ...
    await chat.generate(text, async (_: number, message: string) => {
      const response = await WebModel.processResponse(this.intelliChat, messages, {text: message});
  ...
  private async generateResp(messages: Messages, pMessages: MessageContentI[], chat: WebLLM.ChatInterface) {
    ...
    const {body, error} = await RequestUtils.processRequestInterceptor(this.intelliChat, {body: {text: lastText}});
  ...
  private static async processResponse(intelliChat: intelliChat, messages: Messages, output: ResponseI) {
    const result: ResponseI = (await intelliChat.responseInterceptor?.(output)) || output;
    if (result.error) {
      RequestUtils.displayError(messages, new Error(result.error));
      return;
    } else if (!result || !result.text) {
      const error = ErrorMessages.INVALID_MODEL_RESPONSE(output, !!intelliChat.responseInterceptor, result);
      RequestUtils.displayError(messages, new Error(error));
      return;
    }
    return result;
  ...
  private static clearAllCache() {
    // IMPORTANT - 'webllm/model' and 'webllm/wasm' need to match the scope in 'intelli-chat-web-llm':
