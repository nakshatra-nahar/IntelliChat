...
public class CohereService {
  ...
  public intelliChatTextRespose chat(intelliChatRequestBody requestBody) throws Exception {
    ...
    return new intelliChatTextRespose(responseBody.getText());
  }

  private static CohereChatBody createChatBody(intelliChatRequestMessage[] messages) {
    ...
    for (int i = 0; i < messages.length - 1; i++) {
      intelliChatRequestMessage message = messages[i];
  ...
  public intelliChatTextRespose generateText(intelliChatRequestBody requestBody) throws Exception {
    LOGGER.info("Received request body: {}", PrintObjectUtil.toJsonString(requestBody));
    ...
    // Sends response back to intelli Chat using the Response format:
    // https://intellichat.dev/docs/connect/#Response
    return new intelliChatTextRespose(responseBody.getGenerations()[0].getText());
  }

  public intelliChatTextRespose summarizeText(intelliChatRequestBody requestBody) throws Exception {
    LOGGER.info("Received request body: {}", PrintObjectUtil.toJsonString(requestBody));
    ...
    // Sends response back to intelli Chat using the Response format:
    // https://intellichat.dev/docs/connect/#Response
    return new intelliChatTextRespose(responseBody.getSummary());
  }
}
