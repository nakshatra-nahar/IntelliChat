...
import com.server.utils.types.intelliChatRequestMessage;
...
import com.server.utils.types.intelliChatFile;
...
public class OpenAIService {
  ...
  public intelliChatTextRespose chat(intelliChatRequestBody requestBody) throws Exception {
    ...
    return new intelliChatTextRespose(responseBody.getChoices()[0].getMessage().getContent());
  }

  @SuppressWarnings("unchecked")
  public Flux<intelliChatTextRespose> chatStream(intelliChatRequestBody requestBody) {
    LOGGER.info("Received request body: {}", PrintObjectUtil.toJsonString(requestBody));
      ...
      .concatMap(data -> {
        try {
          ...
          try {
            ...
            return Flux.just(new intelliChatTextRespose(""));
          }
          ...
          // Sends response back to intelli Chat using the Response format:
          // https://intellichat.dev/docs/connect/#Response
          return Flux.just(new intelliChatTextRespose(delta.toString()));
  ...
  public intelliChatFileResponse imageVariation(@RequestPart("files") List<MultipartFile> files) throws Exception {
    ...
    // Sends response back to intelli Chat using the Response format:
    // https://intellichat.dev/docs/connect/#Response
    return new intelliChatFileResponse(Arrays.asList(new intelliChatFile(responseData.getData()[0].getUrl(), "image")));
  }
}
