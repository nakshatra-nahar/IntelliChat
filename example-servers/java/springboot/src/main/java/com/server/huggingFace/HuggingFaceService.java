...
public class HuggingFaceService {
  ...
  public intelliChatTextRespose conversation(intelliChatRequestBody requestBody) throws Exception {
    ...
    return new intelliChatTextRespose(responseBody.getGeneratedText());
  }

  private static HuggingFaceConversationBody createConversationBody(intelliChatRequestMessage[] messages) {
  ...
  public intelliChatTextRespose imageClassification(@RequestPart("files") List<MultipartFile> files) throws Exception {
    ...
    // Sends response back to intelli Chat using the Response format:
    // https://intellichat.dev/docs/connect/#Response
    return new intelliChatTextRespose(responseData[0].getLabel());
  }
  ...
  public intelliChatTextRespose speechRecognition(@RequestPart("files") List<MultipartFile> files) throws Exception {
    ...
    // Sends response back to intelli Chat using the Response format:
    // https://intellichat.dev/docs/connect/#Response
    return new intelliChatTextRespose(responseData.getText());
  }
}
