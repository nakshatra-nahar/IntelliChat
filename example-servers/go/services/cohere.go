...
func createCohereChatRequestBodyBytes(w http.ResponseWriter, r *http.Request, stream bool) ([]byte, error) {
	...
	var intelliChatRequestBody intelliChatRequestBody
	err = json.Unmarshal(userRequestBodyBytes, &intelliChatRequestBody)
	if err != nil {
	...
	chatBody := createCohereChatBody(intelliChatRequestBody.Messages)
	return json.Marshal(chatBody)
}

func createCohereChatBody(intelliChatRequestMessages []intelliChatRequestMessage) CohereChatBody {
	return CohereChatBody{
		Query: intelliChatRequestMessages[len(intelliChatRequestMessages)-1].Text,
		ChatHistory: func() []CohereChatHistoryMessage {
			chatHistory := make([]CohereChatHistoryMessage, 0, len(intelliChatRequestMessages)-1)
			for _, message := range intelliChatRequestMessages[:len(intelliChatRequestMessages)-1] {
				userName := "USER"
				if message.Role == "ai" {
					userName = "CHATBOT"
				}
...
func CohereGenerate(w http.ResponseWriter, r *http.Request) error {
	...
	fmt.Println("Request Body:", intelliChatRequestBody.Messages[0].Text)

	generateBody := CohereGenerateBody{
		Prompt: intelliChatRequestBody.Messages[0].Text,
	}
...
func CohereSummarize(w http.ResponseWriter, r *http.Request) error {
	...
	summarizeBody := CohereSummarizeBody{
		Text: intelliChatRequestBody.Messages[0].Text,
	}
