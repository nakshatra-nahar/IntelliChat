...
func createChatRequestBodyBytes(w http.ResponseWriter, r *http.Request, stream bool) ([]byte, error) {
	...
	chatBody := createChatBody(intelliChatRequestBody, stream)
	return json.Marshal(chatBody)
}

func createChatBody(intelliChatRequestBody intelliChatRequestBody, stream bool) OpenAIChatBody {
	chatBody := OpenAIChatBody{
		Messages: make([]OpenAIChatMessage, len(intelliChatRequestBody.Messages)),
		Model:    intelliChatRequestBody.Model,
	}
	for i, message := range intelliChatRequestBody.Messages {
		role := message.Role
...
// You can use an example image here: https://github.com/OvidijusParsiunas/intelli-chat/blob/main/example-servers/ui/assets/example-image-for-openai.png
